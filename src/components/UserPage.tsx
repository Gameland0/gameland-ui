...
export const UserPage = () => {
  ...
  const setCollectionPie = () => {
    ...
    simplehash.get(`https://api.simplehash.com/api/v0/nfts/owners?chains=arbitrum&wallet_addresses=${useraddress}&limit=50`).then((vals) => {
      const addressArr = [] as any
  ...
  const getTokensData = async () => {
    ...
    polygonhttp.get(`v0/oklink/addressBalance?chainShortName=arbitrum&address=${useraddress}&protocolType=token_20`).then((vals) => {
      const Tokensoptionsdata = [] as any
      let tokenTotal = 0
      vals.data.data[0].tokenList.map((item: any) => {
        if (item.priceUsd * 1 > 0) {
    ...
    if (PieChartData && PieChartData.length) {
      ...
      PieChartData?.map((item: any) => {
          ...
          item.actions.map((ele: any) => {
            if (ele.type === 'trade') {
              ...
              Tabledata.push({
                ...
                chain: chains,
                platform: item.platform,
                type: ele.from?.toLowerCase() === useraddress?.toLowerCase() ? 'Sold' : 'Bought',
                time: new Date(item.timestamp*1000).toJSON().substr(0, 10)
              })
            }
          })
        ...
        if (item.tag === 'exchange') {
          item.actions.map((ele: any) => {
            if (ele.from?.toLowerCase() === useraddress?.toLowerCase()) {
              if (ele.type === 'swap') {
  ...
  const SnapshotChartClick = (params: any) => {
    const filterData = PieChartData.filter((item: any) => {
      return item.platform==='Snapshot'&&new Date(item.timestamp*1000).toJSON().substr(5, 5)===params.name
    })
    setLensModalData(filterData)
    setShowLensModal(true)
  }
          ...
          {PopUpsData && PopUpsData.length ? (
            PopUpsData.map((item: any, index: number) => (
              <div className="spacing-5" key={index}>
                ...
                <span className="blue">{formatting(item.from)}</span>&nbsp;&nbsp;
                ...
                <b>
                  {item.type === 'revise'
                    ...
                    ? ''
                    : item.type === 'swap'
                    ? new BigNumber(item.actions[0].metadata?.from.value).div(`1e${item.actions[0].metadata?.from.decimals}`).toNumber().toFixed(2) +
                      ' ' +
                ...
                &nbsp;
                {item.type === 'swap'
                  ? new BigNumber(item.actions[0].metadata?.to.value).div(`1e${item.actions[0].metadata?.to.decimals}`).toNumber().toFixed(2) +
                    ' ' +
                    item.actions[0].metadata.to.symbol
                  : <span className="blue">{formatting(item.to || '0x000')}</span>}
                &nbsp;&nbsp;on {item.network}
              </div>
            ))
          ) : (
